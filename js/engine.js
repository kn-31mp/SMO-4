...
(function() {
    ...
    calculateHighLoadParams = function() {
        const highLoadForm = document.forms[HIGH_LOAD_FORM_TITLE];
        const highLoadParams = _retrieveHighLoadFormParams(highLoadForm);
        const outputHtml = _getOutputHtml(highLoadParams);

        const resultElement = document.getElementById(HIGH_LOAD_CALCULATION_OUTPUT_ELEMENT_ID);
        resultElement.innerHTML = outputHtml;

        location.href = `#${HIGH_LOAD_CALCULATION_OUTPUT_ELEMENT_ID}`;
    }

    function _getOutputHtml(highLoadParams) {
        let output = '<div class="mb-3"><b>РЕЗУЛЬТАТИ ОБЧИСЛЕНЬ</b></div>';
        ...
        output += channelReceiveSummaryHtml;
        ...
        output += serverOverloadProbabilitySummaryHtml;
    ...
    function _getAggregateChannelReceiveSummaryHtml(aggregateChannelReceiveResult) {
        ...
        const lambda = aggregateChannelReceiveResult['lambda'].toFixed(PRECISION);
        summaryHtml += `<li>Інтенсивність надходження заявок від користувачів (заявок/сек): ${lambda}</li>`;

        const timeToReceiveRequest = aggregateChannelReceiveResult['timeToReceiveRequest'].toFixed(PRECISION);
        summaryHtml += `<li>Час отримання однієї заявки (сек): ${timeToReceiveRequest}</li>`;

        const channelLoad = aggregateChannelReceiveResult['channelLoad'].toFixed(PRECISION);
        summaryHtml += `<li>Завантаженість агрегованого каналу прийому (норма <1): ${channelLoad}</li>`;

        const coefficientOfChannelUsage = aggregateChannelReceiveResult['coefficientOfChannelUsage'].toFixed(PRECISION);
        ...
        if (coefficientOfChannelUsage >= 1) {
            ...
            const averageQueueLength = aggregateChannelReceiveResult['averageQueueLength'].toFixed(PRECISION);
    ...
    function _getServerOverloadProbabilitySummaryHtml(serverOverloadProbabilityResult) {
        ...
        const avgServerRequestProcessTime = serverOverloadProbabilityResult['avgServerRequestProcessTime'].toFixed(PRECISION);
        summaryHtml += `<li>Середній час обслуговування заявки сервером (с): ${avgServerRequestProcessTime}</li>`;

        const serverRequestProcessIntensity = serverOverloadProbabilityResult['serverRequestProcessIntensity'].toFixed(PRECISION);
        summaryHtml += `<li>Інтенсивність обробки заявок сервером (заявок/сек): ${serverRequestProcessIntensity}</li>`;

        const serverOverloadProbability = serverOverloadProbabilityResult['serverOverloadProbability'].toFixed(PRECISION);
    ...
    function _getAggregateChannelSendSummaryHtml(aggregateChannelSendResult) {
        ...
        const receiveIntensity = aggregateChannelSendResult['receiveIntensity'].toFixed(PRECISION);
        summaryHtml += `<li>Інтенсивність надходження заявок від сервера (заявок/сек): ${receiveIntensity}</li>`;

        const avgTransmissionTime = aggregateChannelSendResult['avgTransmissionTime'].toFixed(PRECISION);
        summaryHtml += `<li>Час передавання однієї заявки (сек): ${avgTransmissionTime}</li>`;

        const transmissionChannelLoad = aggregateChannelSendResult['transmissionChannelLoad'].toFixed(PRECISION);
        summaryHtml += `<li>Завантаженість агрегованого каналу передачі: ${transmissionChannelLoad}</li>`;

        const transmissionChannelUsage = aggregateChannelSendResult['transmissionChannelUsage'].toFixed(PRECISION);
        summaryHtml += `<li>Коефіцієнт використання агрегованого каналу передачі: ${transmissionChannelUsage}</li>`;

        const trafficTransmissionIntensity = aggregateChannelSendResult['trafficTransmissionIntensity'].toFixed(PRECISION);
        ...
        if (transmissionChannelUsage >= 1) {
            ...
            const avgQueueLength = aggregateChannelSendResult['avgQueueLength'].toFixed(PRECISION);
    ...
    function _getGeneralSummaryHtml(generalResult) {
        ...
        const coefficientOfChannelUsage = generalResult['coefficientOfChannelUsage'].toFixed(PRECISION);
        const serverOverloadProbability = generalResult['serverOverloadProbability'].toFixed(PRECISION);
        const transmissionChannelUsage = generalResult['transmissionChannelUsage'].toFixed(PRECISION);
        if ((coefficientOfChannelUsage >= 1) || (serverOverloadProbability >= 1) || (transmissionChannelUsage >= 1)) {
            ...
            const userWaitTime = generalResult['userWaitTime'].toFixed(PRECISION);
            summaryHtml += `<li>Час, який користувач чекає на відповідь: ${userWaitTime}</li>`;

            const userWaitTimeMin = generalResult['userWaitTimeMin'].toFixed(PRECISION);
            summaryHtml += `<li>Мінімальний час, за який користувач отримує відповідь (без черги): ${userWaitTimeMin}</li>`;
        }

        const avgReceiveSpeed = generalResult['avgReceiveSpeed'].toFixed(PRECISION);
        summaryHtml += `<li>Середня швидкість прийому: ${avgReceiveSpeed}</li>`;

        const avgSendSpeed = generalResult['avgSendSpeed'].toFixed(PRECISION);
        ...
        if (channelType == SIMPLEX_CHANNEL_TYPE) {
            const receiveSpeed = generalResult['receiveSpeed'].toFixed(PRECISION);
            summaryHtml += `<li>Максимальна швидкість приймання: ${receiveSpeed}</li>`;

            const sendSpeed = generalResult['sendSpeed'].toFixed(PRECISION);
    ...
    calculateHighLoadParams = calculateHighLoadParams
})();
